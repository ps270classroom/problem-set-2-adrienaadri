...
## Answer 1
```{r}
fb_ads <- read_csv("/Users/ellis/OneDrive/Documents/problem-set-2-adrienaadri/data/fb_ads.csv")
fb_ads
```
...
knitr::kable(party_incumbent_promote, col.names = c("Party", "Incumbency", "Average Promoting Ads "), digits = 3)
```
...
## Answer 3
The histogram looks as if the number of candidates decreases exponentially as impressions_millions increases. Most of the House candidates had less than 10 million ads impressions on Facebook.
...
fb_top_plot <- fb_ads |>
  ...
  ggplot(aes(x = impressions_millions,
             y = reorder(cand_name_last, impressions_millions))) +
  geom_bar(stat = "identity", aes(fill = party)) +
  labs(
    title = "Top 15 House Candidates Facebook Ad Impressions",
